...
%DIF LATEXDIFF DIFFERENCE FILE
%DIF DEL hnl_prd_main.tex                                                       Thu Aug 13 15:34:40 2026
%DIF ADD /Users/nickkamp/Downloads/IceCubeFirstHNL_PRD_v2.0a/hnl_prd_main.tex   Tue Jan 13 09:36:48 2026
    aps,
    prd,
    reprint,
    amsmath,
...
We find that the identification of these events against the background is challenging, leading \DIFdelbegin \DIFdel{to }\DIFdelend a substantial difference between the model parameter space range where a single event is expected~\cite{Coloma:2017ppo, Boiarska:2021yho} \DIFaddbegin \DIFadd{to }\DIFaddend and our resulting limits.
...
An essential part of this work was the development of an event generator\DIFaddbegin \DIFadd{, which was extensively used }\DIFaddend to study the signal of HNLs in IceCube\DIFdelbegin \DIFdel{with a }\DIFdelend \DIFaddbegin \DIFadd{, crucially factoring in the }\DIFaddend realistic detector response.
...
\DIFdelbegin \DIFdel{We note here that due to differences between the }\textsc{\DIFdel{GENIE}} %DIFAUXCMD
\DIFdel{and }\textsc{\DIFdel{NuXSSplMkr}} %DIFAUXCMD
\DIFdel{cross section models, the HNL production cross section at low energies and HNL masses can slightly exceed the SM NC cross section in \mbox{%DIFAUXCMD
...
For this analysis, LI-HNL is run in volume mode, and two generators are used to produce half of the events from $\nu_\tau$ and the other half from $\bar{\nu}_\tau$ as the primary particle. Volume mode injects the primary particles within a cylindrical volume, where the center and the dimension are chosen at runtime. Here, the volume is chosen to be centered in the DeepCore sub-array, and we produce samples with masses of \SI{0.1}{\gev}, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\SI{0.3}{\gev}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{SI}{\DIFadd{0.3}}{\gev}\DIFaddend , \SI{0.6}{\gev}, and \SI{1.0}{\gev}. The 0.1 GeV sample was excluded from the final analysis due to the dominance of the invisible 3-neutrino decay mode, which pushed the vast majority of events simulated below DeepCore’s energy detection threshold.
...
\DIFdelbegin \DIFdel{We note that the weighting scheme of }\textsc{\DIFdel{LI-HNL}} %DIFAUXCMD
\DIFdel{does not account for modifications to the neutral current potential experienced by atmospheric neutrinos traversing the Earth in the presence of nonzero $|U_{\tau 4}|^2$.
...
%DIFDELCMD < %%%
\DIFdelend %%%%%%%%%%%%%%%%%%%% Event Filtering and Reconstruction %%%%%%%%%%%%%%%%%%%%
...
\begin{table}[h]
    ...
    \DIFaddbeginFL \DIFaddFL{$\nu^\rm{NC}$       }& \DIFaddFL{0.0666 }& \DIFaddFL{19491 $\pm$ 47 }\\
    \DIFaddendFL $\nu_\tau^\rm{CC}$  & 0.0345 & 10094 $\pm$ 22 \\
    \DIFdelbeginFL \DIFdelFL{$\nu_\mu^\rm{NC}$   }%DIFDELCMD < & %%%
\DIFdelFL{0.0443 }%DIFDELCMD < & %%%
\DIFdelFL{12965  $\pm$ 42 }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{$\nu_e^\rm{NC}$     }%DIFDELCMD < & %%%
\DIFdelFL{0.0110 }%DIFDELCMD < & %%%
\DIFdelFL{3213   $\pm$ 18 }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{$\nu_\tau^\rm{NC}$  }%DIFDELCMD < & %%%
\DIFdelFL{0.0113 }%DIFDELCMD < & %%%
\DIFdelFL{3313   $\pm$ 13 }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelendFL $\mu_\rm{atm}$      & 0.0032 & 936 $\pm$ 15 \\
    \hline
...
\begin{figure}[h]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{rates_with_nutaunc.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{signal_rates_and_events.png}
\DIFaddendFL \caption{HNL rates and event expectations for all three masses at final level of the event selection\DIFdelbeginFL \DIFdelFL{, as well as the $\nu_{\tau}$ NC rate at final level}\DIFdelendFL .\DIFdelbeginFL \DIFdelFL{Note that the maximum $\nu_{\tau}$ rate does not meet the maximum HNL rate - this is due to the greater selection efficiency for HNLs at final level. }\DIFdelendFL }
...
\DIFdelbegin \DIFdel{The background is evaluated at $|U_{\tau 4}|^2=0$.
...
%DIFDELCMD < %%%
\DIFdelend During the binning scheme development we found that some lower energy bins in the cascade-like PID bin have small expected numbers of events, and can not be adequately described by Gaussian statistics.
...
\DIFdelbegin \DIFdel{Recall that the mixing matrix element $|U_{\tau 4}|^2$ enters in both the $N_i^{\rm HNL}$ term as well as the $\nu_\tau$ NC component of the background term $N_i^\nu$.
...
The included nuisance parameters stem from several sources of systematic uncertainties related to both event generation and detector response. Following the work presented in Ref.~\cite{IceCubeCollaboration:2023wtb}, the physical models are used to implement the uncertainties on event distributions, resulting from varying these systematic parameters. For the cases where no analytical description is available, effective parameters, calculated based on Monte \DIFdelbegin \DIFdel{Carlo }\DIFdelend \DIFaddbegin \DIFadd{Carlos }\DIFaddend simulations, are used. We list here only the systematic uncertainties that were found to be relevant for this analysis, while all other parameters are kept fixed at their nominal values~\cite{Fischer_2023}.

All uncertainties that scale the total neutrino expectation are included in a single scaling parameter $N_{\nu}$, because the analysis is primarily sensitive to \DIFdelbegin \DIFdel{shape difference between HNL and neutrino eventswithin the PID and $\cos(\textrm{zenith})$ bins.
...
}\DIFdelend \DIFaddbegin \DIFadd{the relative distribution of events, while the total flux of neutrinos is not relevant for the measurement itself.
...
\begin{table}[h]
    \begin{tabular}{ ccccc }
      ...
      %  \hline\hline
       ...
       % \hline
        \hline\hline
       \textbf{HNL mass} & \textbf{\ut4} & \textbf{68\% CL} & \textbf{90\% CL} \DIFdelbeginFL %DIFDELCMD < & %%%
\textbf{\DIFdelFL{NH $p$-value}}  %DIFAUXCMD
\DIFdelendFL \\    
       \hline\hline
       \SI{0.3}{\gev} & 0.001 & 0.08 & \DIFdelbeginFL \DIFdelFL{0.17  }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\SI{0.44}{}}\hskip0pt%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.67  }\DIFaddendFL \\
        \SI{0.6}{\gev} & \DIFdelbeginFL \DIFdelFL{$8 \times 10^{-5}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{8 \times 10^{-5} }\DIFaddendFL & 0.22 & 0.36  \DIFdelbeginFL %DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\SI{0.39}{}}\hskip0pt%DIFAUXCMD
}\DIFdelendFL \\
        \SI{1.0}{\gev} & 0.054 & \DIFdelbeginFL \DIFdelFL{0.27 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.22 }\DIFaddendFL & 0.44  \DIFdelbeginFL %DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\SI{0.44}{} }\hskip0pt%DIFAUXCMD
}\DIFdelendFL \\
       \hline
