...
%DIF DEL /Users/nickkamp/Downloads/IceCubeFirstHNL_PRD_v2.0a/hnl_prd_main.tex   Tue Jan 13 03:36:48 2026
%DIF ADD hnl_prd_main.tex                                                       Fri Sep 25 12:47:35 2026
    aps,
    prd,
    reprint,
    amsmath,
...
\DIFdelbegin \DIFdel{.
}\DIFdelend In this article, we consider the production of HNLs from high-energy atmospheric neutrinos interacting in the Antarctic ice, with subsequent decay of the HNLs, which could be detected with the IceCube Neutrino Observatory~\cite{IceCube:2016zyt}, a possibility first considered in Ref.~\cite{Coloma:2017ppo}.
...
We find that the identification of these events against the background is challenging, leading \DIFaddbegin \DIFadd{to }\DIFaddend a substantial difference between the model parameter space range where a single event is expected~\cite{Coloma:2017ppo, Boiarska:2021yho} \DIFdelbegin \DIFdel{to }\DIFdelend and our resulting limits.
...
An essential part of this work was the development of an event generator \DIFdelbegin \DIFdel{, which was extensively used }\DIFdelend to study the signal of HNLs in IceCube \DIFdelbegin \DIFdel{, crucially factoring in the }\DIFdelend \DIFaddbegin \DIFadd{with a }\DIFaddend realistic detector response.
...
This is shown in \cref{fig:total_cross_section}, where the cross-sections generated for this analysis are compared to the SM NC \DIFdelbegin \DIFdel{cross-sections }\DIFdelend \DIFaddbegin \DIFadd{cross sections }\DIFaddend calculated using \textsc{GENIE}~\cite{Andreopoulos:2009rq}.
\DIFaddbegin \DIFadd{We note here that due to differences between the }\textsc{\DIFadd{GENIE}} \DIFadd{and }\textsc{\DIFadd{NuXSSplMkr}} \DIFadd{cross section models, the HNL production cross section at low energies and HNL masses can slightly exceed the SM NC cross section in \mbox{%DIFAUXCMD
\cref{fig:total_cross_section}}\hskip0pt%DIFAUXCMD
.
While this is not physical, it is a small effect and is covered by a dedicated DIS systematic parameter assigned to the difference between the two cross section models described in \mbox{%DIFAUXCMD
\cref{sec:uncertainties}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
For this analysis, LI-HNL is run in volume mode, and two generators are used to produce half of the events from $\nu_\tau$ and the other half from $\bar{\nu}_\tau$ as the primary particle. Volume mode injects the primary particles within a cylindrical volume, where the center and the dimension are chosen at runtime. Here, the volume is chosen to be centered in the DeepCore sub-array, and we produce samples with masses of \SI{0.1}{\gev}, \DIFdelbegin \DIFdel{SI}%DIFDELCMD < {%%%
\DIFdel{0.3}%DIFDELCMD < }{\gev}%%%
\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\SI{0.3}{\gev}}\hskip0pt%DIFAUXCMD
}\DIFaddend , \SI{0.6}{\gev}, and \SI{1.0}{\gev}. The 0.1 GeV sample was excluded from the final analysis due to the dominance of the invisible 3-neutrino decay mode, which pushed the vast majority of events simulated below DeepCore’s energy detection threshold.
...
\DIFaddbegin \DIFadd{We note that the weighting scheme of }\textsc{\DIFadd{LI-HNL}} \DIFadd{does not account for modifications to the neutral current potential experienced by atmospheric neutrinos traversing the Earth in the presence of nonzero $|U_{\tau 4}|^2$.
This can modify the $\nu_\mu \to \nu_\tau$ oscillation probability, though the effect is expected to be sub-leading~\mbox{%DIFAUXCMD
\cite{Sahoo:2023mpj} }\hskip0pt%DIFAUXCMD
and will therefore not change the conclusions of this article.
}


\DIFaddend %%%%%%%%%%%%%%%%%%%% Event Filtering and Reconstruction %%%%%%%%%%%%%%%%%%%%
...
In the case of a non-zero \ut4, this results in \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend shift of events from SM neutral-current tau-neutrino interactions to HNLs, which have different energy and zenith distributions compared to the SM case. We use this shift to investigate the $m_4$-\ut4 space.
...
\DIFdelFL{0.0666 }%DIFDELCMD < & %%%
\DIFdelFL{19491 $\pm$ 47 }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelendFL $\nu_\tau^\rm{CC}$  & 0.0345 & 10094  $\pm$ 22 \\
    \DIFaddbeginFL \DIFaddFL{$\nu_\mu^\rm{NC}$   }& \DIFaddFL{0.0443 }& \DIFaddFL{12965  $\pm$ 42 }\\
    \DIFaddFL{$\nu_e^\rm{NC}$     }& \DIFaddFL{0.0110 }& \DIFaddFL{3213   $\pm$ 18 }\\
    \DIFaddFL{$\nu_\tau^\rm{NC}$  }& \DIFaddFL{0.0113 }& \DIFaddFL{3313   $\pm$ 13 }\\
    \DIFaddendFL $\mu_\rm{atm}$      & 0.0032 & 936    $\pm$ 15 \\
    \hline
...
\begin{figure}[h]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{signal_rates_and_events.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{rates_with_nutaunc.pdf}
\DIFaddendFL \caption{HNL rates and event expectations for all three masses at final level of the event selection\DIFaddbeginFL \DIFaddFL{, as well as the $\nu_{\tau}$ NC rate at final level}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{Note that the maximum $\nu_{\tau}$ rate does not meet the maximum HNL rate - this is due to the greater selection efficiency for HNLs at final level. }\DIFaddendFL }
...
\DIFaddbegin \DIFadd{The background is evaluated at $|U_{\tau 4}|^2=0$.
Note that the $\nu_\tau$ NC component of the background reduces for $|U_{\tau 4}|^2>0$, as shown in \mbox{%DIFAUXCMD
\cref{fig:signal_expecations}}\hskip0pt%DIFAUXCMD
; however, this is a sufficiently small component of the overall background compared to the $\nu_\mu$ and $\nu_e$ components that the total background distribution is nearly independent of $|U_{\tau 4}|^2$.
}

\DIFaddend During the binning scheme development we found that some lower energy bins in the cascade-like PID bin have small expected numbers of events, and can not be adequately described by Gaussian statistics.
These bins are therefore removed from the analysis and illustrated in dark green in \cref{fig:background_3d_hist} and all similar figures that follow. The data observed in \SI{9.28}{years} of live time, binned in the same scheme, is shown in \cref{fig:data_3d_hist}\DIFaddbegin \DIFadd{.
}\DIFaddend 
...
All uncertainties that scale the total neutrino expectation are included in a single scaling parameter $N_{\nu}$, because the analysis is primarily sensitive to \DIFdelbegin \DIFdel{the relative distribution of events , while the total flux of neutrinos is not relevant for the measurement itself.
}\DIFdelend \DIFaddbegin \DIFadd{shape difference between HNL and neutrino events within the PID and $\cos(\textrm{zenith})$ bins.
...
}\DIFaddend Three parameters, $h_{\pi^+}$, $i_{\pi^+}$, and $y_{K^+}$, are used to account for the uncertainties in the pion and kaon production in the atmosphere, which are the dominant sources of neutrino production at the energies considered in this work.
...
\begin{table}[h]
    \begin{tabular}{ ccccc }
      ...
      %  \hline\hline
       ...
       % \hline
        \hline\hline
       \textbf{HNL mass} & \textbf{\ut4} & \textbf{68\% CL} & \textbf{90\% CL} \DIFaddbeginFL & \textbf{\DIFaddFL{NH $p$-value}}  \DIFaddendFL \\    
       \hline\hline
       \SI{0.3}{\gev} & 0.001 & 0.08 & \DIFdelbeginFL \DIFdelFL{0.67  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.17  }& \DIFaddFL{\mbox{%DIFAUXCMD
\SI{0.44}{}}\hskip0pt%DIFAUXCMD
}\DIFaddendFL \\
        \SI{0.6}{\gev} & \DIFdelbeginFL \DIFdelFL{$8 \times 10^{-5}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$8 \times 10^{-5}$ }\DIFaddendFL & 0.22 & 0.36  \DIFaddbeginFL & \DIFaddFL{\mbox{%DIFAUXCMD
\SI{0.39}{}}\hskip0pt%DIFAUXCMD
}\DIFaddendFL \\
        \SI{1.0}{\gev} & 0.054 & \DIFdelbeginFL \DIFdelFL{0.22 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.27 }\DIFaddendFL & 0.44 \DIFaddbeginFL & \DIFaddFL{\mbox{%DIFAUXCMD
\SI{0.44}{} }\hskip0pt%DIFAUXCMD
}\DIFaddendFL \\
       \hline
    \end{tabular}
...
\end{table}
The observed test statistic (TS) profiles are shown in \cref{fig:brazil_bands}. The TS is calculated as the $\Delta\chi^2_{\mathrm{mod}}$ between a free fit and a fit where the mixing is fixed to a best fit mixing, using $\chi^2_{\mathrm{mod}}$ as defined in \DIFdelbegin \DIFdel{\ref{eq:mod-chi2-hnl}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:mod-chi2-hnl}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and the best-fit points shown in \DIFdelbegin \DIFdel{\ref{tab:best_fit_parameters_and_confidence_levels}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{tab:best_fit_parameters_and_confidence_levels}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
\begin{figure}[h]
    ...
    % \includegraphics[width=\columnwidth]{bfp_3d_binwise_pull_1.0_GeV.pdf}
    	\caption{Three-dimensional bin-wise pulls between data and simulation for a $|U_{\tau 4}|^2$ (\SI{0.6}{\gev}) = \DIFdelbeginFL \DIFdelFL{0.8 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$8 \times 10^{-5}$ }\DIFaddendFL sample fit.}
