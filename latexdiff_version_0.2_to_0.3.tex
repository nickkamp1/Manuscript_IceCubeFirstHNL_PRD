...
%DIF DEL 2024_10_15_version_0.2_hnl_prd_main.tex   Tue Oct 15 08:43:26 2024
%DIF ADD 2024_12_11_version_0.3_hnl_prd_main.tex   Wed Dec 11 14:16:22 2024
    aps,
    prd,
    reprint,
    amsmath,
...
%DIF < \def\nomath_ut4{|U_{\tau4}|^2}
%DIF -------
\def\nomathue4{|U_{e4}|^2} %DIF > 
\def\nomathum4{|U_{\mu4}|^2} %DIF > 
\def\nomathut4{|U_{\tau4}|^2} %DIF > 
%DIF -------
...
\DIFdelbegin %DIFDELCMD < \keywords{sterile neutrinos, right-handed neutrinos, heavy neutal leptons, HNL, IceCube, DeepCoore, atmospheric neutrinos}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \keywords{sterile neutrinos, right-handed neutrinos, heavy neutral leptons, HNLs, IceCube, DeepCore, atmospheric neutrinos}
\DIFaddend 
...
\begin{abstract}
\begin{linenumbers}
The observation of neutrino oscillations has established that neutrinos have non-zero masses. This phenomenon is not explained by the standard model of particle physics, but one viable explanation to this dilemma is the existence of heavy neutral leptons in the form of right-handed neutrinos. This work presents the first search for heavy neutral leptons with the IceCube Neutrino Observatory. The standard three flavor neutrino model is extended by adding a fourth GeV-scale mass state allowing mixing with the $\tau$ sector through the parameter \ut4. The analysis is performed by searching for \DIFdelbegin \DIFdel{an excess of cascade-like events from }\DIFdelend \DIFaddbegin \DIFadd{signatures of }\DIFaddend heavy neutral leptons \DIFdelbegin \DIFdel{on top of the standard model neutrino sample, leveraging neutrino energies from \mbox{%DIFAUXCMD
\SIrange{5}{100}{\gev}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{that are directly produced via up-scattering of atmospheric $\nu_\tau$'s inside the IceCube detection volume.
%DIF >  an excess of cascade-like events from heavy neutral leptons on top of the standard model neutrino sample, leveraging neutrino energies from \SIrange{5}{100}{\gev}.
}\DIFaddend Three heavy neutral lepton mass values, $m_4$, of \SI{0.3}{\gev}, \SI{0.6}{\gev}, and \SI{1.0}{\gev} are tested using ten years of data, collected between 2011 and 2021. No significant signal of heavy neutral leptons is observed for any of the tested masses. The resulting constraints for the mixing parameter are \DIFdelbegin \DIFdel{$\nomath_ut4 < 0.19\;(m_4 = \SI{0.3}{\gev})$, $\nomath_ut4 < 0.36\;(m_4 = \SI{0.6}{\gev})$, and $\nomath_ut4 < 0.40\;(m_4 = \SI{1.0}{\gev})$ }\DIFdelend \DIFaddbegin \DIFadd{$\nomathut4 < 0.19\;(m_4 = \SI{0.3}{\gev})$, $\nomathut4 < 0.36\;(m_4 = \SI{0.6}{\gev})$, and $\nomathut4 < 0.40\;(m_4 = \SI{1.0}{\gev})$ }\DIFaddend at \SI{90}{\percent} confidence level. \DIFaddbegin \DIFadd{This analysis serves as proof-of-concept for heavy neutral lepton searches in IceCube. }\DIFaddend The developed signal generator and the \DIFaddbegin \DIFadd{thorough }\DIFaddend investigation of the expected signatures lay the fundamental groundwork for future searches \DIFdelbegin \DIFdel{for heavy neutral leptons in IceCube}\DIFdelend \DIFaddbegin \DIFadd{thereof}\DIFaddend .
\end{linenumbers}
\end{abstract}
...
An HNL can be produced from the interactions of neutrinos through the weak force, since an HNL has a small component of left-handed neutrino flavors, leading to \textit{weaker-than-weak} interactions contributing to their production and \DIFdelbegin \DIFdel{decays}\DIFdelend \DIFaddbegin \DIFadd{decay}\DIFaddend .
Since HNL mixing \DIFdelbegin \DIFdel{to the }\DIFdelend \DIFaddbegin \DIFadd{with active }\DIFaddend neutrinos is presumed to be small, as it is yet unobserved, this leads to long HNL lifetimes unless new forces are in action.
In this article, we consider the production of HNLs from high-energy atmospheric neutrinos interacting and decaying in the Antarctic ice, which could be detected with the IceCube Neutrino Observatory~\cite{IceCube:2016zyt}, a possibility first considered in Ref.~\cite{Coloma:2017ppo}.
\DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{Production of HNLs from the decay of mesons has also been proposed in Refs.~\mbox{%DIFAUXCMD
\cite{Boiarska:2021yho,Coloma:2019htx,Arguelles:2019ziu}}\hspace{0pt}%DIFAUXCMD
; in particular, Ref.~\mbox{%DIFAUXCMD
\cite{Boiarska:2021yho} }\hspace{0pt}%DIFAUXCMD
proposes to look for HNLs using down-going events in IceCube and KM3NeT.
...
In particular, when considering only upward-going events and HNLs in the GeV-scale, the production of meson decay is a sub-leading production mechanism compared to the one proposed in Ref.~\mbox{%DIFAUXCMD
...
An additional }\DIFaddend unique advantage of \DIFdelbegin \DIFdel{this setup is }\DIFdelend \DIFaddbegin \DIFadd{looking for upward-going events is the }\DIFaddend access to a relatively large flux of $\nu_\tau$\DIFaddbegin \DIFadd{s }\DIFaddend produced from the oscillation of $\nu_\mu$\DIFaddbegin \DIFadd{s }\DIFaddend with energies in the tens of GeVs~\cite{IceCubeCollaboration:2023wtb}, where the neutrino cross-section is well-described by deep-inelastic scattering (DIS)~\cite{Formaggio:2012cpf}.
...
\DIFaddbegin \DIFadd{Within our setup, Ref.~\mbox{%DIFAUXCMD
\cite{Coloma:2017ppo} }\hspace{0pt}%DIFAUXCMD
predicted IceCube to detect one event when the mixing was $\nomathut4\sim10^{-4}$ for a mass of \mbox{%DIFAUXCMD
\SI{1.0}{\gev}}\hspace{0pt}%DIFAUXCMD
, while Ref.~\mbox{%DIFAUXCMD
\cite{Boiarska:2021yho} }\hspace{0pt}%DIFAUXCMD
made similar claims for KMNeT/ORCA.
However, as we will discuss in this article, this rate does not readily translate to constraints at this mixing level due to large background and particle identification errors. 
}\DIFaddend 
...
The emission of light in the first process is governed by the inelasticity of the neutrino interaction and light produced in the hadronic shower, while the emission of light in the second process depends on the decay channel.
Since hadronic showers cannot be resolved as such in IceCube, they are observed as cascade-like light depositions.
\cref{fig:hnl_branching_ratios} shows the branching fractions of an HNL with mass $m_4$ into SM products, using the calculation given in Ref.~\cite{Coloma:2020lgy}.
\DIFdelbegin \DIFdel{Most }\DIFdelend \DIFaddbegin \DIFadd{The majority }\DIFaddend of these decay modes \DIFdelbegin \DIFdel{also }\DIFdelend produce a cascade-like light deposition in IceCube, where just \DIFdelbegin \DIFdel{signature of the decay mode }\DIFdelend \DIFaddbegin \DIFadd{the decay }\DIFaddend with $\mu$'s in the final state \DIFdelbegin \DIFdel{has an energy dependence. 
}\DIFdelend \DIFaddbegin \DIFadd{can produce elongated light patterns if sufficient energy is transferred to the outgoing $\mu$. 
The dominant decay mode for all HNL masses above the pion production threshold and below \mbox{%DIFAUXCMD
\SI{1}{\gev}}\hspace{0pt}%DIFAUXCMD
, the decay to the neutral pion - $\nu_4 \to \nu_\tau + \pi^0$ - produces a pure EM cascade, for example.
}\DIFaddend 

The \DIFdelbegin \DIFdel{relative light emission between }\DIFdelend \DIFaddbegin \DIFadd{light emissions of }\DIFaddend the first and second processes, together with the potentially large separation between them\DIFaddbegin \DIFadd{, }\DIFaddend can produce a signature composed of two cascade-like light depositions.
\DIFaddbegin \DIFadd{The distance between the two cascades depends on the lifetime of the HNL, which follows an exponential distribution and is strongly dependent on $m_4$ and }\ut4\DIFadd{, which are the free model parameters.
}\DIFaddend Adding this morphological signature of two cascades to the known categories in IceCube makes three distinct morphological categories\DIFdelbegin \DIFdel{observable in IceCube}\DIFdelend \DIFaddbegin \DIFadd{, which are theoretically observable}\DIFaddend : cascades, tracks, and double-cascades. 
\DIFdelbegin \DIFdel{Despite this }\DIFdelend \DIFaddbegin \DIFadd{However, given the sparsity of the detector and the best case energy detection threshold of a few \si{\gev}~\mbox{%DIFAUXCMD
\cite{IceCube:2022kff}}\hspace{0pt}%DIFAUXCMD
, most events of all three categories are observed as single cascades, especially at lower energies.
...
They effectively show up as a modification to the energy/zenith distribution of the cascade component of the sample, in addition to a small component of events that deposit light in the form of the unique low-energy }\DIFaddend double-cascade \DIFaddbegin \DIFadd{signature.
}

\DIFadd{An essential part of this work was the development of an event generator, which was extensively used to study the signal of HNLs in IceCube, crucially factoring in the realistic detector response.
The generator~\mbox{%DIFAUXCMD
\cite{LeptonInjector-HNL} }\hspace{0pt}%DIFAUXCMD
is published as an open-source tool to produce HNL events for atmospheric neutrino detectors. 
Based on the generated events, the performance of detecting and identifying HNLs via the double-cascade channel was investigated, but despite this }\DIFaddend morphology being unique to HNLs, it is not currently \DIFdelbegin \DIFdel{observable with available methods }\DIFdelend \DIFaddbegin \DIFadd{possible to observe and correctly identify it, given the large background of SM neutrino events.
As an alternative avenue and serving as a }\textit{\DIFadd{proof-of-principle}} \DIFadd{search for HNLs }\DIFaddend in IceCube, \DIFdelbegin \DIFdel{as we will discuss later.
Given the position of IceCube to probe HNL-$\nu_\tau$ mixing, }\DIFdelend the \DIFdelbegin \DIFdel{branching ratios in \mbox{%DIFAUXCMD
\cref{fig:hnl_branching_ratios} }\hspace{0pt}%DIFAUXCMD
assume $\nu_\tau$ mixing dominance, i.
e. $\nomath_ut4 \neq 0$ and $|U_{e4}|^2 = |U_{\mu 4}|^2 = 0$.
%DIF <  The visible light emitted in the second cascade critically depends on the allowed decay modes.
}\DIFdelend \DIFaddbegin \DIFadd{excess of cascade-like events stemming from HNLs, on top of the expectation from SM interactions, is used to perform a first measurement of }\ut4 \DIFadd{for three masses with ten years of data from the IceCube DeepCore sub-array~\mbox{%DIFAUXCMD
\cite{IceCube:2011ucd}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 
...
\begin{figure}[h]
    ...
    \caption{Branching ratios of an HNL within the mass range considered in this work, only considering \DIFdelbeginFL \DIFdelFL{$\nomath_ut4 \neq 0$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\nomathut4 \neq 0$}\DIFaddendFL , calculated based on the results from Ref.~\cite{Coloma:2020lgy}.}
    \label{fig:hnl_branching_ratios}
\end{figure}

\DIFdelbegin \DIFdel{In this article , we present the first search of HNLs using the IceCube DeepCore~\mbox{%DIFAUXCMD
\cite{IceCube:2011ucd} }\hspace{0pt}%DIFAUXCMD
sub-array by looking for excesses in cascade and track morphologies relative to the expectation from SM interactions. It }\DIFdelend \DIFaddbegin \DIFadd{The article }\DIFaddend is organized as follows.
In \cref{sec:detector}, we introduce the IceCube DeepCore detector, which is the primary instrument used in this search.
Following this, in \cref{sec:generator}, we describe \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend new event generator\DIFaddbegin \DIFadd{, used }\DIFaddend to simulate HNL events \DIFdelbegin \DIFdel{which was developed }\DIFdelend for this search.
Here also the detector response simulation and the HNL event re-weighting scheme are discussed.
In \cref{sec:filter_and_reco}, we describe the filtering steps leading to a \DIFdelbegin \DIFdel{neutrino dominated }\DIFdelend \DIFaddbegin \DIFadd{neutrino-dominated }\DIFaddend analysis sample, including the \DIFdelbegin \DIFdel{standard DeepCore low-energy }\DIFdelend reconstruction used in the analysis.
\DIFdelbegin \DIFdel{Additionally}\DIFdelend \DIFaddbegin \DIFadd{In particular}\DIFaddend , we explore the \DIFdelbegin \DIFdel{issue }\DIFdelend \DIFaddbegin \DIFadd{performance }\DIFaddend of reconstructing low-energy \DIFdelbegin \DIFdel{spatially separated cascades }\DIFdelend \DIFaddbegin \DIFadd{double-cascades }\DIFaddend with traditional likelihood methods \DIFdelbegin \DIFdel{, showing the limitations of the current methods.
In\mbox{%DIFAUXCMD
\cref{sec:analysis_methods}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{and the resulting capability to identify them.
We find that the identification of these events against the background is challenging, leading to a substantial difference between the range where a single event is expected~\mbox{%DIFAUXCMD
\cite{Coloma:2017ppo,Boiarska:2021yho} }\hspace{0pt}%DIFAUXCMD
to the sensitive region.
In~\mbox{%DIFAUXCMD
\cref{sec:analysis_methods,sec:results}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , we describe the analysis methods and \DIFdelbegin \DIFdel{in \mbox{%DIFAUXCMD
\cref{sec:results} }\hspace{0pt}%DIFAUXCMD
we present the resultsof this work. 
}\DIFdelend \DIFaddbegin \DIFadd{the results, respectively. 
%DIF >  we describe the analysis methods and in \cref{sec:results} we present the results of this work.
}\DIFaddend Finally, in\DIFaddbegin \DIFadd{~}\DIFaddend \cref{sec:conclusion} we conclude and discuss prospects for future HNL searches in IceCube.
...
A dedicated Monte Carlo generator was developed to simulate the signal events used in this analysis. It is an adaptation of the \textsc{LeptonInjector} (LI) package~\cite{IceCube:2020tcq} named \textsc{LeptonInjector-HNL} (LI-HNL)\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{LeptonInjector-HNL}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . LI is a standalone event generator capable of producing neutrino interactions optimized for the needs of neutrino observatories. LI-HNL makes use of the existing structure of LI, modified to support the HNL's beyond standard model physics.
This section outlines LI's basic functionality, listing the changes made to support HNL production, and describes the event generation procedure\DIFaddbegin \DIFadd{, }\DIFaddend including a detailed description of the theoretical signature.
...
LI directly injects the final states of neutrino interactions. For example, in a charged-current (CC) DIS interaction, LI generates a lepton and a hadronic shower at the interaction vertex of the neutrino. It assigns the relevant final state kinematics by sampling from the corresponding differential cross-sections.

Neutrino flux and transport through Earth are handled by a complementary package, \textsc{LeptonWeighter} (LW)\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{IceCube:2020tcq} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend with inputs from \textsc{MCEq}~\cite{Fedynitch:2018cbl} and \textsc{nuSQuIDS}~\cite{Arguelles:2021twb}.
...
After a distance\DIFaddbegin \DIFadd{, }\DIFaddend $L$\DIFaddbegin \DIFadd{,  }\DIFaddend the HNL is forced to decay, and a set of secondary particles are generated at the decay point. The decay also happens via a NC weak interaction.
The \DIFdelbegin \DIFdel{sampling }\DIFdelend distance is chosen from \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an ad-hoc }\DIFaddend power-law distribution, with minimum and maximum values that maximize the selection efficiency in this analysis. \DIFaddbegin \DIFadd{The physically correct distribution will be achieved through the re-weighting process described in \mbox{%DIFAUXCMD
\cref{sec:weighting}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend These parameters are customizable for other detectors' needs.
...
The mass of the HNL is \DIFdelbegin \DIFdel{currently }\DIFdelend a free parameter in the model under investigation. \DIFdelbegin \DIFdel{We are interested in HNL masses in the GeV regime, where modifications of }\DIFdelend \DIFaddbegin \DIFadd{As motivated in }\DIFaddend the \DIFdelbegin \DIFdel{see-saw mechanism can account for lightneutrino masses without introducing complications to the hierarchy problem, as discussed in Refs.
~\mbox{%DIFAUXCMD
\cite{Vissani:1997ys,Coloma:2017ppo}}\hspace{0pt}%DIFAUXCMD
.
...
%DIF >  , where modifications of the see-saw mechanism can account for light neutrino masses without introducing complications to the hierarchy problem, as discussed in Refs.~\cite{Vissani:1997ys,Coloma:2017ppo}.
}\DIFaddend Because the mass affects the production and decay kinematics, as well as the accessible decay modes, multiple discrete mass samples are tested individually.
For the analysis described in this work, three samples are produced for masses of \SI{0.3}{\gev}, \SI{0.6}{\gev}, and \SI{1.0}{\gev}.
The \DIFdelbegin \DIFdel{Generator }\DIFdelend \DIFaddbegin \DIFadd{generator }\DIFaddend additionally supports an HNL mass down to \SI{0.1}{\gev}. The current analysis scheme, however, turned out to be less sensitive to this mass and was not pursued to the end.
...
\begin{figure}[h]
    ...
    \internallinenumbers
    \caption{Total neutrino-nucleon cross-sections for the $\nu_\tau$/$\bar{\nu}_\tau$ up-scattering into the heavy mass state, shown for the three target masses, compared to the total SM $\nu_\tau$/$\bar{\nu}_\tau$-NC cross-sections used for the baseline neutrino simulation production with \textsc{GENIE}. The plot \DIFdelbeginFL \DIFdelFL{assumes }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{shows the cross-sections for }\DIFaddendFL a mixing of \DIFdelbeginFL \DIFdelFL{$\nomath_ut4=1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\nomathut4=1$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{while }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{since }\DIFaddendFL the cross-sections scale linearly with the mixing \ut4 \DIFaddbeginFL \DIFaddFL{and the plot aims to visualize the shape compared to the SM case}\DIFaddendFL .}
...
\begin{figure}[h]
    ...
    \internallinenumbers
    \caption{Shown are decay widths of the HNL within the mass range considered in this work, assuming mixing only with the $\tau$ sector. Since the \DIFdelbeginFL \DIFdelFL{total }\DIFdelendFL decay \DIFdelbeginFL \DIFdelFL{width scales }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{widths scale }\DIFaddendFL linearly with the mixing, it can be factored out and \DIFdelbeginFL \DIFdelFL{here }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the plot shows the decay widths for }\DIFaddendFL a mixing of \DIFdelbeginFL \DIFdelFL{$\nomath_ut4=1$ is assumed}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\nomathut4=1$ to visualize their shape}\DIFaddendFL . Calculated based on the results from Ref.~\cite{Coloma:2020lgy}.}
...
\begin{figure}[h]
    ...
    \internallinenumbers
    \caption{Generation level Bjorken $x$ distribution (top) and inelasticity $y$ distribution \footnote{The $y$ distributions actually used for the analysis looks slightly different, since during post-unblinding checks, a bug was discovered in the spline-fitting procedure of the differential cross-sections that led to inconsistencies in the generated inelasticity distribution. The bug has been fixed in both \textsc{NuXSSplMkr} and LI-HNL, and was shown to have a negligible impact on the final analysis results.}(bottom) for the three mass samples. Shown are the distributions from 25000 MC events, weighted according to the baseline atmospheric flux and oscillation parameters and to a mixing of \DIFdelbeginFL \DIFdelFL{$\nomath_ut4 = 0.001$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\nomathut4 = 0.001$}\DIFaddendFL . The shaded area indicates the statistical uncertainty.}
    \label{fig:bjorken_x_inelasticity_y}
\end{figure}

\cref{fig:bjorken_x_inelasticity_y} shows the generation level distributions of Bjorken $x$ and $y$ for the HNL production. The Bjorken variables have a direct influence on the fraction of visible energy in each event. Visible energy is defined here as the energy contained in the hadronic shower and non-neutrino decay products, which can produce Cherenkov radiation visible in the IceCube detector. The distributions are weighted to a mixing of \DIFdelbegin \DIFdel{$\nomath_ut4 = 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$\nomathut4 = 0.001$}\DIFaddend , which leads to a reduced expected rate for the \SI{1.0}{\gev} case, because the expected decay lengths become larger than the chosen simulation range (compare the example behavior shown in \cref{fig:hnl_decay_feynman_lengths}). It can be seen how a larger HNL mass leads to to Bjorken x distribution peaking at larger values, because the momentum transfer increased, which on the other hand leads to a reduction in the inelasticity, which can be seen by the inelasticity distribution peaking towards smaller values for increasing HNL mass.

\cref{fig:visible_energies} shows the visible energy distribution and the distribution of the visible energy fraction, again weighted to a mixing of \DIFdelbegin \DIFdel{$\nomath_ut4 = 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$\nomathut4 = 0.001$}\DIFaddend .
The peak in the visible energy fraction of the \SI{1.0}{\gev} case around a fraction of 0.4 is a result of the inelasticity distribution peaking at lower values combined with a large proportion of decays into three neutrinos, resulting \DIFdelbegin \DIFdel{solely }\DIFdelend \DIFaddbegin \DIFadd{in only }\DIFaddend the energy from the first cascade being visible.
All visible energy fraction distributions are increasing towards 1.0, but the visible energy of the \SI{0.3}{\gev} case peaks strongest towards 1.0, because its main decay is the $\pi^0$ mode and the invisible three-neutrino decay \DIFdelbegin \DIFdel{becomes }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend subdominant. These plots show that already at generation level, a large portion of the energy is invisible to the detector.
...
\begin{figure}[h]
    ...
    \internallinenumbers
    \caption{Generation level total visible energy distribution (top) and visible energy fraction with respect to the total true neutrino energy (bottom). Shown are the distributions from 25000 MC events, weighted according to the baseline atmospheric flux and oscillation parameters and to a mixing of \DIFdelbeginFL \DIFdelFL{$\nomath_ut4 = 0.001$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\nomathut4 = 0.001$}\DIFaddendFL . The shaded area indicates the statistical uncertainty.    
    }
...
\subsection{Monte Carlo Re-Weighting} \DIFaddbegin \label{sec:weighting}
\DIFaddend 
...
The resulting event sample contains 150,257 events with an atmospheric $\mu$ contamination of \SI{0.6}{\percent}.
...
Notably, \SI{11.3}{\percent} of the sample corresponds to NC interactions, which is relevant for this analysis as this indicates the maximum possible proportion of events from HNL NC up-scattering interactions. For further details on the event selection used in this analysis we refer the reader to Refs.~\cite{IceCubeCollaboration:2023wtb,IceCube:2024xjj}.

\begin{figure}[h]
    \includegraphics[width=\columnwidth]{rates_per_level_hnl.png}
    \internallinenumbers
    \caption{Selection efficiency of the filter levels aiming at reducing atmospheric $\mu$'s and noise. Shown are the rates of all standard model backgrounds as well as the HNL rates for the three mass samples at a mixing of \DIFdelbeginFL \DIFdelFL{$\nomath_ut4 = 0.1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\nomathut4 = 0.1$}\DIFaddendFL .}
...
Events from low-energy atmospheric neutrinos either produce a pure cascade ($\nu_{e}$-CC, \SI{83}{\percent} $\nu_{\tau}$-CC all $\nu$-NC) or a track and cascade ($\nu_{\mu}$-CC, \SI{17}{\percent} $\nu_{\tau}$-CC) morphology. The production and the decay of an HNL inside the detector\DIFaddbegin \DIFadd{, }\DIFaddend however, can produce two cascade-like light depositions. The algorithm used to attempt to reconstruct and identify this morphology is based on the reconstruction method used to search for high-energy, astrophysical $\nu_{\tau}$'s~\cite{Abbasi:2020zmr} and was first introduced in Refs.~\cite{PHallen, MUsner}. Like most traditional IceCube reconstructions~\cite{IceCube:2013dkx,IceCube:2024csv}, it uses a maximum likelihood algorithm, comparing the light observed in the detector to the expected light distribution for a given event hypothesis, through a Poisson likelihood. Different event hypotheses can be tested, where the parameter set defining the specific event is $\Theta$. The double-cascade hypothesis is composed of two cascades aligned with the neutrino direction separated by a certain distance. Assuming the HNL is relativistic, this geometry can be defined by nine parameters: the position of the first cascade, $x, y, z$, the direction of the neutrino, $\phi, \theta$, the interaction time, $t$, and the distance from first to second cascade, $L$, and the energy of each cascade, $E_0$ and $E_1$. Thus, the nine parameters defining an event are given by $\Theta = (x, y, z, t, \theta, \phi, E_0, E_1, L)$.

The energy reconstruction was found to perform well for $E_0$ and $E_1$ above true cascade energies of around \SIrange{8}{10}{\gev}. Below this energy, little or even no light is observed in the detector and the reconstruction performs worse or fails. Due to the kinematic effects explained in~\cref{sec:generator}, a large fraction of the second cascade energies are smaller than this and as a result, for many events, only light from the first cascade was observed and therefore reconstructible. Overall, mostly single photo-electrons are detected in each DOM, while the total number of observed photo-electrons in an event peaks below 10.

As a consequence of this, the decay length resolution shown in \cref{fig:taupede_reconstructed_decay_length}, only performs well for true decay lengths from \SIrange{20}{80}{\meter}, which can be seen by the median being almost unbiased on top of the diagonal. Nonetheless, the spread is still large at the order of $\pm \SI{40}{\meter}$ in the \SI{68}{\percent} band. Outside this region the reconstruction performs poorly, with the median not following the diagonal and above true decay lengths of \SI{80}{\meter}, in particular, more than \SI{60}{\percent} of the reconstructed decay lengths are below \SI{60}{\meter}. With increasing true decay lengths this fraction increases even more and it was found that these events were cases where only one cascade was observed with sufficient light to be reconstructed, while the other remained invisible, making it impossible to reconstruct the distance between the two.
...
In an attempt to identify HNL events over the standard model background from neutrinos and $\mu$'s, the results from this reconstruction were fed into a multivariate machine learning method. The applied method was the Gradient boosting machine~\cite{BDT} from the \textsc{scikit-learn} package~\cite{sklearn} and further details of can be found in Ref.~\cite{Fischer2024First}. A minimum reconstructed length of \SI{40}{\meter} and reconstructed cascade energies above \SI{5}{\gev} was required for all events, while only a subset of signal events with true lengths between \SIrange{40}{150}{\meter} was used, significantly reducing their numbers. As a result of this, and given the overall limited reconstruction performance, the classification of the signal events over the large number of background events from SM atmospheric neutrino interactions was not successful.



%DIF > %%%%%%%%%%%%%%%%%%% Analysis Methodology %%%%%%%%%%%%%%%%%%%%
\DIFaddbegin 

\section{\label{sec:analysis_methods}\DIFadd{Analysis Methodology}}

\DIFaddend Theoretically, HNLs can produce a unique, low-energy \DIFdelbegin \DIFdel{, }\DIFdelend double-cascade signature. Despite this fact, it was not possible to properly reconstruct them with the current selection and reconstruction tools available. Due to the lack of an effective reconstruction, we could then not classify the double-cascade events in a separate morphological category, to perform an analysis searching for occurrences of this unique signature. In the process it was however identified, that most of the events are observed as single cascades in our detector. Since these events are additionally observed, on top of the standard model background of atmospheric neutrino and $\mu$ events, an analysis scheme was revised, that searches for this excess of mostly cascade-like events, happening on top of the SM backgrounds, without the need of explicitly identifying the events stemming from the BSM process. In this alternative avenue to perform the search for HNLs, the standard reconstruction used for low-energy atmospheric neutrinos is applied to all events, including the signal MC. In the case of a non-zero \ut4, this results in an excess of events from HNLs, observed on top of the expected events from Standard Model interactions of atmospheric neutrinos and $\mu$'s which can be used to investigate the $m_4$-\ut4 space.

%DIF < %%%%%%%%%%%%%%%%%%% Analysis Methodology %%%%%%%%%%%%%%%%%%%%
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\section{%DIFDELCMD < \label{sec:analysis_methods}%%%
\DIFdel{Analysis Methodology}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In this section, we first describe the final sample selection before introducing the binning scheme and the resulting background and signal shapes. After describing the fit method, the included systematic uncertainties are discussed at the end of the section.


\subsection{\label{sec:sample}Sample Selection}
...
Based on those results, an additional BDT classifier is trained to separate neutrinos from atmospheric $\mu$'s.
...
\cref{tab:background_final_level_expectation} shows the nominal rates and the nominal event expectation from Standard Model backgrounds as well as the observed number of data events. The sample is neutrino-dominated, with the majority of events being from $\nu_\mu$-CC interactions. \cref{fig:signal_expecations} shows the signal expectation for the three simulated HNL masses, $m_4$ = {0.3, 0.6, 1.0}\,GeV, where both the rate and the expected number of events in \SI{9.28}{years} of data taking are shown with respect to the mixing. For reference, at a mixing of \DIFdelbegin \DIFdel{$\nomath_ut4=10^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{$\nomathut4=10^{-1}$}\DIFaddend , the HNL signal expectation is at a similar order as the number of expected atmospheric $\mu$'s in the final sample. The total rate scales roughly linearly with the mixing, where effects of the different visible energy fractions for the different mass samples affect the slope (shown in \cref{fig:hnl_branching_ratios_data}). Small mixing results in longer decay lengths, leading to non-linearities due to the limited Monte Carlo, which is only simulated up to a decay length of \SI{1000}{\meter}.
...
\begin{table}[h]
    ...
    \end{tabular}
\internallinenumbers
\caption{Observed data events and nominal rates and nominal event expectations of the SM background particle types and interactions assuming \SI{9.28}{years} of detector live time at final level of the analysis selection. The expected rate is calculated as the sum of the weights $\sum_i\omega_i$, with the stated uncertainty being $\sqrt{\sum_i\omega_i^2}$.}
...
To enhance the sensitivity to the HNL's specific event signature, we further divide the events based on three reconstructed variables.
...
\begin{figure}[h]
    ...
    \internallinenumbers
    \caption{Signal events divided by the square root of the nominal background expectation in \SI{9.28}{years}. This assumes a \SI{1.0}{\gev} HNL mass and a mixing of \DIFdelbeginFL \DIFdelFL{$\nomath_ut4=10^{-1}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\nomathut4=10^{-1}$}\DIFaddendFL .}
...
\cref{fig:s_to_sqrt_b_1.0_GeV_0.1_mixing} shows the expected signal shape in the analysis binning assuming an HNL mass of \SI{1.0}{\GeV} and a mixing of \DIFdelbegin \DIFdel{$\nomath_ut4=10^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{$\nomathut4=10^{-1}$}\DIFaddend . As expected, the signal shows up mostly in the cascade and mixed bins, since for many of the events only a single-cascade deposits light in the detector. A small fraction of the events falls into the track-like bin, which can be explained by them having a more elongated light deposition pattern, due to the two cascades, and therefore being classified as track-like by the CNN. It can also be seen that the signal is concentrated in the lower energy bins, at and below the energy at which the $\nu_\tau$ flux peaks. As discussed in detail in \cref{sec:generator}, a fraction of the energy goes into invisible particles and does not deposit any light, therefore lowering the reconstructed energy of the events compared to the energy of the incoming $\nu_\tau$. The distribution of the signal events into cascade-like, mixed, and track-like bin is dependent on both the mixing value as well as the mass, because both modify the proper lifetime of the HNL and with it the decay length in the detector. For very short and very large decay lengths, the events look more cascade-like, while for intermediate lengths, the events can be more elongated, leading to a classification as more track-like. Distributions for all masses and a two mixings of \DIFdelbegin \DIFdel{$\nomath_ut4=10^{-1}$ and $\nomath_ut4=10^{-3}$ }\DIFdelend \DIFaddbegin \DIFadd{$\nomathut4=10^{-1}$ and $\nomathut4=10^{-3}$ }\DIFaddend can be found in \cref{app:signal_shapes} in addition to ratio comparisons of the different mass samples, highlighting their kinematic differences.
...
\end{align*}
They are all compatible with the null hypothesis (NH) of no mixing, i.e., \DIFdelbegin \DIFdel{$\nomath_ut4=0.0$}\DIFdelend \DIFaddbegin \DIFadd{$\nomathut4=0.0$}\DIFaddend .
...
Unfortunately, given the low light yield and \DIFaddbegin \DIFadd{the }\DIFaddend detector performance, \DIFaddbegin \DIFadd{with the current tools }\DIFaddend we are not able to successfully identify low-energy double-cascade events \DIFdelbegin \DIFdel{with current tools.
Given this}\DIFdelend \DIFaddbegin \DIFadd{against the background from SM atmospheric neutrino interactions.
The fact that we presently cannot single out double-cascade events from the background implies that the sensitive region obtained by assuming an order one detected even presented in~\mbox{%DIFAUXCMD
\cite{Coloma:2017ppo} }\hspace{0pt}%DIFAUXCMD
is significantly different from the sensitive region.
Given these findings}\DIFaddend , we instead perform an analysis looking for excess events in the energy and angular distributions using standard low-energy morphological categories in IceCube.
...
We place upper limits on the $\tau$-mixing of HNLs assuming Wilks' theorem, these are:
\DIFdelbegin \DIFdel{$\nomath_ut4 < 0.19\;(m_4 = \SI{0.3}{\gev})$, $\nomath_ut4 < 0.36\;(m_4 = \SI{0.6}{\gev})$, and $\nomath_ut4 < 0.40\;(m_4 = \SI{1.0}{\gev})$ }\DIFdelend \DIFaddbegin \DIFadd{$\nomathut4 < 0.19\;(m_4 = \SI{0.3}{\gev})$, $\nomathut4 < 0.36\;(m_4 = \SI{0.6}{\gev})$, and $\nomathut4 < 0.40\;(m_4 = \SI{1.0}{\gev})$ }\DIFaddend at \SI{90}{\percent} confidence level.
Despite these constraints being several orders of magnitude below the current leading limits on \ut4 from reinterpretations of the CHARM and the BEBC results, which are at the order of $10^{-3}$ to $10^{-6}$ in the \SIrange{0.1}{2}{\gev} range~\cite{Orloff:2002de, Boiarska:2021yho, Barouki:2022bkt}, this initial result serves as a successful \DIFdelbegin \DIFdel{proof of concept }\DIFdelend \DIFaddbegin \textit{\DIFadd{proof-of-concept}} \DIFaddend for HNL searches both in IceCube in particular and using atmospheric neutrinos in general.
...
\subsection{Mixing of \DIFdelbegin \DIFdel{$\nomath_ut4=10^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{$\nomathut4=10^{-1}$}\DIFaddend }
...
\begin{figure}[h!]
    ...
    \includegraphics[width=\columnwidth]{labeled_s_to_sqrt_b_1_0_GeV_combined_U_tau4_sq_0.1000_total.png}
    \internallinenumbers
    \caption{Signal events divided by the square root of the nominal background expectation in \SI{9.28}{years} for all three HNL masses (\SI{0.3}{\gev} top, \SI{0.6}{\gev} middle, \SI{1.0}{\gev} bottom) and a mixing of \DIFdelbeginFL \DIFdelFL{$\nomath_ut4=10^{-1}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\nomathut4=10^{-1}$}\DIFaddendFL .}
...
\begin{figure}[h!]
    ...
    \internallinenumbers
    \caption{Expected signal events of the \SI{0.3}{\gev} (top) and \SI{0.6}{\gev} (bottom) samples divided by the expected events of the \SI{1.0}{\gev} sample in \SI{9.28}{years} for a mixing of \DIFdelbeginFL \DIFdelFL{$\nomath_ut4=10^{-1}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\nomathut4=10^{-1}$}\DIFaddendFL .}
