...
%DIF DEL IceCubeFirstHNL_PRD_v1.1/hnl_prd_main.tex   Tue Jan 13 03:36:32 2026
%DIF ADD IceCubeFirstHNL_PRD_v2.0/hnl_prd_main.tex   Tue Feb 17 16:31:04 2026
    aps,
    prd,
    reprint,
    amsmath,
...
Adding this morphological signature of two cascades to the known categories in IceCube makes three distinct morphological categories \DIFdelbegin \DIFdel{, }\DIFdelend which are theoretically observable: cascades, tracks, and double-cascades.
...
%DIF > This means that the majority of HNL events, which theoretically produce double-cascades, actually appear in the atmospheric neutrino sample as an excess of events looking like cascades.
This means that the majority of HNL events, which theoretically produce double-cascades, actually appear in the atmospheric neutrino sample as \DIFdelbegin \DIFdel{an excess of events looking like cascades.
They effectively appear as }\DIFdelend a shift in the energy and zenith \DIFdelbegin \DIFdel{distribution of the cascade component of the sample, accompanied by }\DIFdelend \DIFaddbegin \DIFadd{distributions of events looking like cascades. Additionally, }\DIFaddend a small subset of events \DIFaddbegin \DIFadd{appear }\DIFaddend with a signature composed of two cascades. This distinctive signature is unique at low energies (in the 10s of GeV), and will from here on be referred to as \textit{low-energy double-cascade} signature.
%DIF > They effectively appear as a shift in the energy and zenith distribution of the cascade component of the sample, accompanied by a small subset of events with a signature composed of two cascades. This distinctive signature is unique at low energies (in the 10s of GeV), and will from here on be referred to as \textit{low-energy double-cascade} signature.
...
As an alternative avenue and serving as a \textit{proof-of-principle} search for HNLs in IceCube, the \DIFdelbegin \DIFdel{excess of }\DIFdelend \DIFaddbegin \DIFadd{shifts in }\DIFaddend cascade-like events stemming from HNLs, \DIFdelbegin \DIFdel{on top of }\DIFdelend \DIFaddbegin \DIFadd{compared to }\DIFaddend the expectation from SM interactions \DIFaddbegin \DIFadd{only}\DIFaddend , is used to perform a first measurement of \ut4 for three masses with ten years of data from the IceCube DeepCore sub-array~\cite{IceCube:2011ucd}.
...
\DIFdelendFL \DIFaddbeginFL \begin{figure}[t!]
    \DIFaddendFL \includegraphics[width=\columnwidth]{dc_geometry.png}
...
\begin{figure}[h]
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.9\columnwidth, trim=0 3.5cm 0 0, clip]{LI-HNL flow-1.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.9\columnwidth, trim=0 3.5cm 0 0, clip]{LI-HNL_flow-1.pdf}
    \DIFaddendFL \caption{Event generation procedure in \textsc{LeptonInjector-HNL}.
...
For this analysis, LI-HNL is run in volume mode, and two generators are used to produce half of the events from $\nu_\tau$ and the other half from $\bar{\nu}_\tau$ as the primary particle. Volume mode injects the primary particles within a cylindrical volume, where the center and the dimension are chosen at runtime. Here, the volume is chosen to be centered in the DeepCore sub-array, and we produce samples with masses of \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\SI{0.1}{\gev}}\hskip0pt%DIFAUXCMD
, }\DIFaddend \SI{0.3}{\gev}, \SI{0.6}{\gev}, and \SI{1.0}{\gev}. \DIFaddbegin \DIFadd{The 0.1 GeV sample was excluded from the final analysis due to the dominance of the invisible 3-neutrino decay mode, which pushed the vast majority of events simulated below DeepCore’s energy detection threshold.
}

\DIFaddend Both the production and the decay of the HNL are presumed to occur inside the detector, which is achieved by placing the primary interaction point inside the detector and limiting the decay length range to be below the size of the detector, which is $\sim$\SI{1000}{\meter}. The parameters chosen for the sample generation are listed in \cref{tab:sampling_dists}. 
...
\DIFdelbegin \DIFdel{As a demonstrative example, the distributions are weighted to a mixing of $\nomathut4 = 0.001$, which leads to a reduced expected rate for the \mbox{%DIFAUXCMD
\SI{1.0}{\gev} }\hskip0pt%DIFAUXCMD
...
\DIFdelbegin \DIFdel{The peak in the visible energy fraction of the \mbox{%DIFAUXCMD
\SI{1.0}{\gev} }\hskip0pt%DIFAUXCMD
case around a fraction of 0.4 is a result of the inelasticity distribution peaking at lower values combined with a large proportion of decays into three neutrinos, resulting in only the energy from the first cascade being visible.
}\DIFdelend %DIF > The peak in the visible energy fraction of the \SI{1.0}{\gev} case around a fraction of 0.4 is a result of the inelasticity distribution peaking at lower values combined with a large proportion of decays into three neutrinos, resulting in only the energy from the first cascade being visible.
All visible energy fraction distributions are increasing towards 1.0, but the visible energy of the \SI{0.3}{\gev} case peaks \DIFdelbegin \DIFdel{strongest }\DIFdelend \DIFaddbegin \DIFadd{most strongly }\DIFaddend towards 1.0, because its main decay is the $\pi^0$ mode and the invisible three-neutrino decay is subdominant. These plots show that already at generation level, a large portion of the energy is invisible to the detector.
...
The horizontal position of the DOMs is not used, since the string pattern is irregular \DIFaddbegin \DIFadd{- a limitation of the CNN architecture used}\DIFaddend .
...
Theoretically, HNLs can produce a unique, low-energy double-cascade signature. Despite this fact, it was not possible to properly reconstruct them with the current selection and reconstruction tools available. Due to the lack of an effective reconstruction, we could then not classify the double-cascade events in a separate morphological category, to perform an analysis searching for occurrences of this unique signature. \DIFdelbegin \DIFdel{In the }\DIFdelend \DIFaddbegin \DIFadd{However, during this }\DIFaddend process it was \DIFdelbegin \DIFdel{however identified , }\DIFdelend \DIFaddbegin \DIFadd{identified }\DIFaddend that most of the events are observed as single cascades in our detector.
These events are observed \DIFdelbegin \DIFdel{on top of }\DIFdelend \DIFaddbegin \DIFadd{together with }\DIFaddend the SM background of atmospheric neutrino and $\mu$ events. For this reason, an analysis scheme was revised \DIFdelbegin \DIFdel{, }\DIFdelend that searches for this \DIFdelbegin \DIFdel{excess }\DIFdelend \DIFaddbegin \DIFadd{shift in distribution }\DIFaddend of mostly cascade-like events, without the need of explicitly identifying the events stemming from the BSM process.
In this alternative avenue to perform the search for HNLs, the standard reconstruction used for low-energy atmospheric neutrinos is applied to all events, including the signal MC, to perform a forward-folding-style analysis. %DIF > In the case of a non-zero \ut4, this results in an excess of events from HNLs, observed on top of the expected events from SM interactions of atmospheric neutrinos and $\mu$'s which can be used to investigate the $m_4$-\ut4 space.
In the case of a non-zero \ut4, this results in an \DIFdelbegin \DIFdel{excess }\DIFdelend \DIFaddbegin \DIFadd{shift }\DIFaddend of events from \DIFdelbegin \DIFdel{HNLs, observed on top of the expected events from SM interactions of atmospheric neutrinos and $\mu$'s which can be used }\DIFdelend \DIFaddbegin \DIFadd{SM neutral-current tau-neutrino interactions to HNLs, which have different energy and zenith distributions compared to the SM case. We use this shift }\DIFaddend to investigate the $m_4$-\ut4 space.
...
%DIF > \cref{fig:s_to_sqrt_b_1.0_GeV_0.1_mixing} shows the expected signal shape in the analysis binning assuming an HNL mass of \SI{1.0}{\GeV} and a mixing of $\nomathut4=10^{-1}$. 
\cref{fig:s_to_sqrt_b_1.0_GeV_0.1_mixing} shows the expected signal shape in the analysis binning assuming an HNL mass of \SI{1.0}{\GeV} and a mixing of $\nomathut4=10^{-1}$. As expected, the signal shows up mostly in the cascade and mixed bins, since for many of the events only a single-cascade deposits light in the detector. \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{We see a shift of a }\DIFaddend small fraction of the events \DIFdelbegin \DIFdel{falls }\DIFdelend into the track-like bin, which can be explained by them having a more elongated light deposition pattern, due to the two cascades, and therefore being classified as track-like by the CNN. It can also be seen that the signal is concentrated in the lower energy bins, at and below the energy at which the $\nu_\tau$ flux peaks. As discussed in detail in \cref{sec:generator}, a fraction of the energy goes into invisible particles and does not deposit any light, therefore lowering the reconstructed energy of the events compared to the energy of the incoming $\nu_\tau$. The distribution of the signal events into the cascade-like, mixed, and track-like bins is dependent on both the mixing value as well as the mass, because both modify the proper lifetime of the HNL and with it the decay length in the detector. For very short and very large decay lengths, the events \DIFdelbegin \DIFdel{look }\DIFdelend \DIFaddbegin \DIFadd{are reconstructed as }\DIFaddend more cascade-like, while for intermediate lengths, the events can be more elongated, leading to a classification as more track-like. Distributions for all masses and a two mixings of $\nomathut4=10^{-1}$ and $\nomathut4=10^{-3}$ can be found in \cref{app:signal_shapes} in addition to ratio comparisons of the different mass samples, highlighting their kinematic differences.
...
The included nuisance parameters stem from several sources of systematic uncertainties related to both event generation and detector response. Following the work presented in Ref.~\cite{IceCubeCollaboration:2023wtb}, the physical models are used to implement the uncertainties on event distributions, resulting from varying these systematic parameters. For the cases where no analytical description is available, effective parameters, calculated based on Monte \DIFdelbegin \DIFdel{Carlos }\DIFdelend \DIFaddbegin \DIFadd{Carlo }\DIFaddend simulations, are used. We list here only the systematic uncertainties that were found to be relevant for this analysis, while all other parameters are kept fixed at their nominal values~\cite{Fischer_2023}.

All uncertainties that scale the total neutrino expectation are included in a single scaling parameter $N_{\nu}$, because the analysis is primarily sensitive to \DIFdelbegin \DIFdel{the relative distribution of events , while the total flux of neutrinos is not relevant for the measurement itself}\DIFdelend \DIFaddbegin \DIFadd{shape difference between HNL and neutrino events within the PID and $\cos(\textrm{zenith})$ bins.
...
Specifically, the $\nu_\tau$ NC cross section is reduced by a factor $(1 - |U_{\tau 4}|^2)$ to preserve unitarity}\DIFaddend.
...
\begin{align*}
    \nomathut4(\SI{0.3}{\gev}) &= \DIFdelbegin \DIFdel{0.003 }\DIFdelend \DIFaddbegin \DIFadd{0.001 }\DIFaddend \;, \\
    \nomathut4(\SI{0.6}{\gev}) &= \DIFdelbegin \DIFdel{0.080 }\DIFdelend \DIFaddbegin \DIFadd{8 \times 10^{-5} }\DIFaddend \;, \rm{and} \\
    \nomathut4(\SI{1.0}{\gev}) &= \DIFdelbegin \DIFdel{0.106 }\DIFdelend \DIFaddbegin \DIFadd{0.054 }\DIFaddend \;.
\end{align*}
%DIF > We provide the $p$-value to reject the NH for each mass in \cref{tab:best_fit_parameters_and_confidence_levels}, along with the upper limits on \ut4. 
We provide the \DIFdelbegin \DIFdel{$p$-value to reject the NH }\DIFdelend \DIFaddbegin \DIFadd{upper limits on }\ut4 \DIFaddend for each mass in \cref{tab:best_fit_parameters_and_confidence_levels}\DIFdelbegin \DIFdel{, along with the upper limits on }%DIFDELCMD < \ut4%%%
\DIFdelend \DIFaddbegin \DIFadd{. Our result is compatible with the null hypothesis, which has good data-MC agreement}\DIFaddend . 
\begin{figure}[h]
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\columnwidth, trim=0 0.5cm 0 0.5cm, clip]{brazil_band_with_asimov_0.3_GeV_updated_with_bfp_with_1sigma.png}
%DIFDELCMD <     \includegraphics[width=0.95\columnwidth, trim=0 0.5cm 0 0.5cm, clip]{brazil_band_with_asimov_0.6_GeV_updated_with_bfp_with_1sigma.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\columnwidth, trim=0 0.5cm 0 0.5cm, clip]{brazil_band_0.3_no_best_fit.png}
    \includegraphics[width=0.95\columnwidth, trim=0 0.5cm 0 0.5cm, clip]{brazil_band_0.6_no_best_fit.png}
    \DIFaddendFL \includegraphics[width=0.95\columnwidth, trim=0 0.5cm 0 0.5cm, clip]{brazil_band_with_asimov_1.0_GeV_updated_with_bfp_with_1sigma.png}
% \begin{figure*}[h]
    ...
    \caption[Best fit point TS profiles]{\DIFdelbeginFL \DIFdelFL{Best fit point }\DIFdelendFL TS profiles as a function of \ut4 for the \SI{0.3}{\gev} (top), \SI{0.6}{\gev} (middle), and \SI{1.0}{\gev} (bottom) mass samples. Shown are the observed profiles, the \DIFdelbeginFL \DIFdelFL{Asimov expectation at the best fit point, and the }\DIFdelendFL \SI{68}{\percent} and \SI{90}{\percent} bands \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL based on 100 pseudo-data trials\DIFaddbeginFL \DIFaddFL{, and the Asimov expectation at the best fit point in the 1.0 GeV case}\DIFaddendFL . Also indicated are the \SI{68}{\percent} and the \SI{90}{\percent} confidence levels assuming Wilks' theorem (horizontal dotted lines).}
    %DIF >  \caption[Best fit point TS profiles]{Best fit point TS profiles as a function of \ut4 for the \SI{0.3}{\gev} (top), \SI{0.6}{\gev} (middle), and \SI{1.0}{\gev} (bottom) mass samples. Shown are the observed profiles, the Asimov expectation at the best fit point, and the \SI{68}{\percent} and \SI{90}{\percent} bands, based on 100 pseudo-data trials. Also indicated are the \SI{68}{\percent} and the \SI{90}{\percent} confidence levels assuming Wilks' theorem (horizontal dotted lines).}
...
%DIFDELCMD <         \textbf{HNL mass} & \textbf{\ut4} & \textbf{68\% CL} & \textbf{90\% CL} & \textbf{NH $p$-value} \\    
%DIFDELCMD <         \hline\hline
%DIFDELCMD <         \SI{0.3}{\gev} & 0.003 & 0.09 & 0.19 & \SI{0.97}{} \\
%DIFDELCMD <         \SI{0.6}{\gev} & 0.080 & 0.21 & 0.36 & \SI{0.79}{} \\
%DIFDELCMD <         \SI{1.0}{\gev} & 0.106 & 0.24 & 0.40 & \SI{0.63}{} \\
%DIFDELCMD <         \hline
...
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{ ccccc }
      ...
      %  \hline\hline
       ...
       \hline
    \end{tabular}
    \DIFaddendFL \caption{Best-fit mixing values and the corresponding upper limits at the \SI{68}{\percent} and the \SI{90}{\percent} confidence level, as well as the $p$-value to reject the null hypothesis, estimated using Wilks' theorem.}
    \label{tab:best_fit_parameters_and_confidence_levels}
\end{table}
The observed test statistic (TS) profiles are shown in \cref{fig:brazil_bands}. The TS is calculated as the $\Delta\chi^2_{\mathrm{mod}}$ between a free fit and a fit where the mixing is fixed to a \DIFdelbegin \DIFdel{given value}\DIFdelend \DIFaddbegin \DIFadd{best fit mixing, using $\chi^2_{\mathrm{mod}}$ as defined in \ref{eq:mod-chi2-hnl}, and the best-fit points shown in \ref{tab:best_fit_parameters_and_confidence_levels}}\DIFaddend .
...
\DIFaddbegin \DIFadd{Because the data are consistent with the null, these can be interpreted as indicating 'preferred regions' in the one-dimensional mixing space of }[\DIFadd{0, upper bound}] \DIFadd{for a given confidence level.
}

\DIFaddend The observed profiles lie within the \SI{68}{\percent} band, confirming their compatibility with statistical fluctuations of the data.
...
\begin{table}[h]
    \DIFdelbeginFL %DIFDELCMD < \begin{tabular}{ ll lll }
%DIFDELCMD <     \hline\hline
%DIFDELCMD <     \textbf{Parameter} & \textbf{Nominal} & \multicolumn{3}{c}{\textbf{Best-Fit / \textit{Pull} [\si{\sigma}]}} \\ 
%DIFDELCMD <     m$_{4}$ (fixed) & - & \textbf{\SI{0.3}{\gev}} & \textbf{\SI{0.6}{\gev}} &  \textbf{\SI{1.0}{\gev}} \\ 
%DIFDELCMD <     \hline\hline
%DIFDELCMD <     $|U_{\tau 4}|^2$                                 & -                    & 0.003               & 0.080               & 0.106               \\
%DIFDELCMD <     GOF p-value                                      & -                    & \SI{28.3}{\percent} & \SI{28.7}{\percent} & \SI{26.0}{\percent} \\
%DIFDELCMD <     \hline
%DIFDELCMD < 

%DIFDELCMD <     $\theta_{23} [\si{\degree}]$                     &    47.5              &          48.1       &          47.9       &          48.0       \\ 
%DIFDELCMD <     $\Delta m^{2}_{32} [\SI{e-3}{\electronvolt^2}] $ &    2.401             &              2.380  &              2.380  &              2.381  \\ 
%DIFDELCMD <     \hline
%DIFDELCMD <     $N_{\nu}$                                        &    1.0               &           0.889     &           0.889     &           0.890     \\ 
%DIFDELCMD < 

%DIFDELCMD <     $\Delta \gamma_\nu$                              &    0.0$\pm$0.10      &          \textit{-0.079}    &          \textit{-0.067}    &          \textit{-0.066}      \\ 
%DIFDELCMD <     $h_{\pi^+}$                                      &    0.0$\pm$0.15      &          \textit{-0.98}     &          \textit{-0.99}     &          \textit{-0.99}      \\ 
%DIFDELCMD <     $i_{\pi^+}$                                      &    0.0$\pm$0.61      &           \textit{0.78}     &          \textit{0.84}      &           \textit{0.86}      \\ 
%DIFDELCMD <     $y_{K^+}$                                        &    0.0$\pm$0.30      &           \textit{0.25}     &          \textit{0.21}      &           \textit{0.19}      \\ 
%DIFDELCMD <     \hline
%DIFDELCMD <     $\rm{DIS}$                                       &    0.0$\pm$1.0       &          \textit{-0.25}     &          \textit{-0.22}     &          \textit{-0.22}      \\ 
%DIFDELCMD <     $M_\rm{A,QE}$                                    &    0.0$\pm$1.0       &          \textit{-0.17}     &          \textit{-0.13}     &          \textit{-0.12}      \\ 
%DIFDELCMD <     $M_\rm{A,res}$                                   &    0.0$\pm$1.0       &          \textit{-0.13}     &          \textit{-0.08}     &          \textit{-0.07}      \\ 
%DIFDELCMD <     \hline
%DIFDELCMD <     $\epsilon_{\rm{DOM}}$                            &    1.0$\pm$0.1       &           \textit{0.22}     &           \textit{0.18}     &           \textit{0.17}      \\ 
%DIFDELCMD < 

%DIFDELCMD <     $\rm{hole \, ice} \; p_0$                        &    0.102             &          -0.161     &          -0.161     &          -0.160     \\ 
%DIFDELCMD <     $\rm{hole \, ice} \; p_1$                        &   -0.049             &          -0.074     &          -0.076     &          -0.076     \\ 
%DIFDELCMD <     $\rm{ice \, absorption}$                         &    1.00              &           0.943     &           0.942     &           0.942     \\ 
%DIFDELCMD <     $\rm{ice \, scattering}$                         &    1.05              &           0.986     &           0.989     &           0.989     \\ 
%DIFDELCMD <     $N_\rm{bfr}$                                     &    0.0               &           0.747     &           0.740     &           0.736     \\
%DIFDELCMD < 

%DIFDELCMD <     \hline
%DIFDELCMD <     \end{tabular}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{ ll lll }
    \hline\hline
...
$N_\rm{bfr}$                                     &    0.0               &           0.747 &           0.740 &           0.736 \\                                     &    0.0               &           0.747     &           0.740     &           0.736     \\
    ...
    \hline
    \end{tabular}
\DIFaddendFL \caption{Best fit nuisance parameters for the three mass samples. Shown are the nominal values, the best-fit values for parameters without a prior, or the pull in units of $\sigma$ for parameters that have a (Gaussian) prior.}
\label{tab:best_fit_parameters}
\end{table}

\begin{figure}[h]
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.9\columnwidth, trim=0 0.5cm 0 1.5cm, clip]{reco_energy_data_mc_agreement.png}
%DIFDELCMD <     \includegraphics[width=0.9\columnwidth, trim=0 0.5cm 0 1.5cm, clip]{reco_coszen_data_mc_agreement.png}
%DIFDELCMD <     \includegraphics[width=0.9\columnwidth, trim=0 0.5cm 0 1.5cm, clip]{pid_data_mc_agreement.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.9\columnwidth, trim=0 0.5cm 0 0.5cm, clip]{reco_energy_data_mc_agreement.png}
    ...
    \DIFaddendFL \caption{Distributions of reconstructed energy, cosine of the reconstructed zenith angle, and PID of the data (black points) compared to the best fit MC shown as stacked histograms for different particle interaction types. The HNL events, which assume an HNL mass of \DIFdelbeginFL \DIFdelFL{0.6 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.0 }\DIFaddendFL GeV, are included as part of the $\nu_\mathrm{NC}$ events in the stacked histogram, and also shown separately multiplied by a factor of 100 in gold.}
...
\DIFdelbegin \DIFdel{The goodness-of-fit (GOF) is estimated by comparing the final fit metric value to the distribution of values expected from the ensemble, resulting in a p-value, quantifying the probability of finding this result. 
The GOF p-values are listed in \mbox{%DIFAUXCMD
\cref{tab:best_fit_parameters}}\hskip0pt%DIFAUXCMD
. }\DIFdelend %DIF > The goodness-of-fit (GOF) is estimated by comparing the final fit metric value to the distribution of values expected from the ensemble, resulting in a p-value, quantifying the probability of finding this result. 
%DIF > The GOF p-values are listed in \cref{tab:best_fit_parameters}. 
For further bias checks, bin-wise pulls between data and \DIFdelbegin \DIFdel{the best-fit }\DIFdelend \DIFaddbegin \DIFadd{a sample }\DIFaddend MC are investigated, illustrated in \cref{fig:3_d_bfp_pull_0.6_GeV} for the $m_{4}=\SI{0.6}{\gev}$ fit.
...
\begin{figure}[h]
    ...
    % \includegraphics[width=\columnwidth]{bfp_3d_binwise_pull_1.0_GeV.pdf}
    	\caption{Three-dimensional bin-wise pulls between data and simulation \DIFdelbeginFL \DIFdelFL{at the best fit point of the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for a $|U_{\tau 4}|^2$ (}\DIFaddendFL \SI{0.6}{\gev}\DIFdelbeginFL \DIFdelFL{mass }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{)  = 0.8 }\DIFaddendFL sample fit.}
...
\DIFdelbegin %DIFDELCMD < \bibliography{hnl_prd_bibfile}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{hnl_prd_bib}
\DIFaddend 
...
%DIF > |U_{\tau4}|^2
%DIF >  \subsection{Mixing of $\nomathut4=10^{-1}$}
%DIF >  \subsection{Mixing of $|U_{\tau4}|^2=10^{-1}$}
\subsection{Mixing of \DIFdelbegin \DIFdel{$\nomathut4=10^{-1}$}\DIFdelend \DIFaddbegin \texorpdfstring{$|U_{\tau4}|^2=10^{-1}$}{|U tau4|2=10-1}\DIFaddend }
...
%DIF > \subsection{Mixing of $\nomathut4=10^{-3}$}
\subsection{Mixing of \DIFdelbegin \DIFdel{$\nomathut4=10^{-3}$}\DIFdelend \DIFaddbegin \texorpdfstring{$|U_{\tau4}|^2=10^{-3}$}{|U tau4|2=10-3}\DIFaddend }
