...
We find that the identification of these events against the background is challenging, leading to a substantial difference between the model parameter space range where a single event is expected~\cite{Coloma:2017ppo, Boiarska:2021yho} and our resulting limits.
...
An essential part of this work was the development of an event generator to study the signal of HNLs in IceCube with a realistic detector response.
...
We note here that due to differences between the \textsc{GENIE} and \textsc{NuXSSplMkr} cross section models, the HNL production cross section at low energies and HNL masses can slightly exceed the SM NC cross section in \cref{fig:total_cross_section}.
While this is not physical, it is a small effect and is covered by a dedicated DIS systematic parameter assigned to the difference between the two cross section models described in \cref{sec:uncertainties}.
...
We note that the weighting scheme of \textsc{LI-HNL} does not account for modifications to the neutral current potential experienced by atmospheric neutrinos traversing the Earth in the presence of nonzero $|U_{\tau 4}|^2$.
This can modify the $\nu_\mu \to \nu_\tau$ oscillation probability, though the effect is expected to be sub-leading~\cite{Sahoo:2023mpj} and will therefore not change the conclusions of this article.
...
In the case of a non-zero \ut4, this results in a shift of events from SM neutral-current tau-neutrino interactions to HNLs, which have different energy and zenith distributions compared to the SM case. We use this shift to investigate the $m_4$-\ut4 space.
...
\begin{table}[h]
    ...
    $\nu_\mu^\rm{NC}$   & 0.0443 & 12965  $\pm$ 42 \\
    $\nu_e^\rm{NC}$     & 0.0110 & 3213   $\pm$ 18 \\
    $\nu_\tau^\rm{NC}$  & 0.0113 & 3313   $\pm$ 13 \\
...
\begin{figure}[h]
\includegraphics[width=\columnwidth]{rates_with_nutaunc.pdf}
\caption{HNL rates and event expectations for all three masses at final level of the event selection, as well as the $\nu_{\tau}$ NC rate at final level. Note that the maximum $\nu_{\tau}$ rate does not meet the maximum HNL rate - this is due to the greater selection efficiency for HNLs at final level. }
...
The background is evaluated at $|U_{\tau 4}|^2=0$.
Note that the $\nu_\tau$ NC component of the background reduces for $|U_{\tau 4}|^2>0$, as shown in \cref{fig:signal_expecations}; however, this is a sufficiently small component of the overall background compared to the $\nu_\mu$ and $\nu_e$ components that the total background distribution is nearly independent of $|U_{\tau 4}|^2$.
...
Recall that the mixing matrix element $|U_{\tau 4}|^2$ enters in both the $N_i^{\rm HNL}$ term as well as the $\nu_\tau$ NC component of the background term $N_i^\nu$.
...
\begin{table}[h]
    \begin{tabular}{ ccccc }
      ...
      %  \hline\hline
       ...
       \textbf{HNL mass} & \textbf{\ut4} & \textbf{68\% CL} & \textbf{90\% CL} & \textbf{NH $p$-value}  \\    
       \hline\hline
       \SI{0.3}{\gev} & 0.001 & 0.08 & 0.17  & \SI{0.44}{}\\
        \SI{0.6}{\gev} & $8 \times 10^{-5}$ & 0.22 & 0.36  & \SI{0.39}{}\\
        \SI{1.0}{\gev} & 0.054 & 0.27 & 0.44 & \SI{0.44}{} \\
       \hline
    \end{tabular}
...
\end{table}
The observed test statistic (TS) profiles are shown in \cref{fig:brazil_bands}. The TS is calculated as the $\Delta\chi^2_{\mathrm{mod}}$ between a free fit and a fit where the mixing is fixed to a best fit mixing, using $\chi^2_{\mathrm{mod}}$ as defined in \cref{eq:mod-chi2-hnl}, and the best-fit points shown in \cref{tab:best_fit_parameters_and_confidence_levels}.
...
\begin{figure}[h]
    ...
    % \includegraphics[width=\columnwidth]{bfp_3d_binwise_pull_1.0_GeV.pdf}
    	\caption{Three-dimensional bin-wise pulls between data and simulation for a $|U_{\tau 4}|^2$ (\SI{0.6}{\gev}) = $8 \times 10^{-5}$ sample fit.}
...
We place upper limits on the $\tau$-mixing of HNLs at $\nomathut4 < 0.17\;(m_4 = \SI{0.3}{\gev})$, $\nomathut4 < 0.36\;(m_4 = \SI{0.6}{\gev})$, and $\nomathut4 < 0.44\;(m_4 = \SI{1.0}{\gev})$ at the \SI{90}{\percent} confidence level.
