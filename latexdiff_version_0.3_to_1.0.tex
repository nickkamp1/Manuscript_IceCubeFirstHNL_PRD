...
%DIF DEL 2024_12_11_version_0.3_hnl_prd_main.tex   Wed Dec 11 14:16:22 2024
%DIF ADD 2025_01_02_version_1.0_hnl_prd_main.tex   Thu Jan  2 09:15:14 2025
    aps,
    prd,
    reprint,
    amsmath,
...
Three heavy neutral lepton mass values, $m_4$, of \SI{0.3}{\gev}, \SI{0.6}{\gev}, and \SI{1.0}{\gev} are tested using ten years of data, collected between 2011 and 2021. No significant signal of heavy neutral leptons is observed for any of the tested masses. The resulting constraints for the mixing parameter are $\nomathut4 < 0.19\;(m_4 = \SI{0.3}{\gev})$, $\nomathut4 < 0.36\;(m_4 = \SI{0.6}{\gev})$, and $\nomathut4 < 0.40\;(m_4 = \SI{1.0}{\gev})$ at \SI{90}{\percent} confidence level. This analysis serves as proof-of-concept for heavy neutral lepton searches in IceCube. The developed signal generator and the \DIFdelbegin \DIFdel{thorough investigation }\DIFdelend \DIFaddbegin \DIFadd{analysis }\DIFaddend of the expected signatures lay the fundamental groundwork for future searches thereof.
\end{linenumbers}
\end{abstract}
...
Since HNL mixing with active neutrinos is presumed to be small, as it is yet unobserved, this leads to long HNL lifetimes unless new forces are in action.
...
Production of HNLs from the decay of mesons has also been proposed in Refs.~\cite{Boiarska:2021yho,Coloma:2019htx,Arguelles:2019ziu}; in particular, Ref.~\cite{Boiarska:2021yho} proposes to look for HNLs using down-going events in IceCube and KM3NeT.
...
In particular, when considering only upward-going events and HNLs in the GeV-scale, the production of meson decay is a sub-leading production mechanism compared to the one proposed in Ref.~\cite{Coloma:2017ppo}.
%As we will discuss, this is the most effective production mechanism for upward-going HNLs in the mass range considered in this article, compared to production from light or heavy meson decays in the atmosphere.
...
Within our setup, Ref.~\cite{Coloma:2017ppo} predicted IceCube to detect one event when the mixing was $\nomathut4\sim10^{-4}$ for a mass of \SI{1.0}{\gev}, while Ref.~\cite{Boiarska:2021yho} made similar claims for KMNeT/ORCA.
...
The light emissions of the first and second processes, together with the potentially large separation between them, can produce a signature composed of two cascade-like light depositions\DIFaddbegin \DIFadd{, therefore called double-cascade signature}\DIFaddend .
...
However, given the \DIFdelbegin \DIFdel{sparsity }\DIFdelend \DIFaddbegin \DIFadd{sparse arrangement }\DIFaddend of the detector \DIFdelbegin \DIFdel{and the best case }\DIFdelend \DIFaddbegin \DIFadd{modules and the }\DIFaddend energy detection threshold of a few \si{\gev}~\cite{IceCube:2022kff} \DIFaddbegin \DIFadd{for events happening within the proximity of the detection modules ($\mathcal{O}(\si{\meter})$)}\DIFaddend , most events of all three categories \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{only deposit light in one location of the detector and are therefore }\DIFaddend observed as single cascades\DIFdelbegin \DIFdel{, especially at lower energies}\DIFdelend .
...
They effectively \DIFdelbegin \DIFdel{show up as a modification to the energy /}\DIFdelend \DIFaddbegin \DIFadd{appear as a shift in the energy and }\DIFaddend zenith distribution of the cascade component of the sample, \DIFdelbegin \DIFdel{in addition to a small component of events that deposit light in the form of the unique low-energy double-cascade }\DIFdelend \DIFaddbegin \DIFadd{accompanied by a small subset of events with a signature composed of two cascades. This distinctive signature is unique at low energies (in the 10s of GeV), and will from here on be referred to as }\textit{\DIFadd{low-energy double-cascade}} \DIFaddend signature.
...
We find that the identification of these events against the background is challenging, leading \DIFdelbegin \DIFdel{to }\DIFdelend a substantial difference between the \DIFaddbegin \DIFadd{model parameter space }\DIFaddend range where a single event is expected~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Coloma:2017ppo,Boiarska:2021yho} }\hspace{0pt}%DIFAUXCMD
to the sensitive region}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Coloma:2017ppo, Boiarska:2021yho} }\hspace{0pt}%DIFAUXCMD
to and our resulting limits}\DIFaddend .
...
A dedicated Monte Carlo generator was developed to simulate the signal events used in this analysis. It is an adaptation of the \textsc{LeptonInjector} (LI) package~\cite{IceCube:2020tcq} named \textsc{LeptonInjector-HNL} (LI-HNL)~\cite{LeptonInjector-HNL}. LI is a standalone event generator capable of producing neutrino interactions optimized for the needs of neutrino observatories. LI-HNL makes use of the existing structure of LI, modified to support the HNL's beyond standard model physics.
...
After a distance, $L$,  the HNL is forced to decay, and a set of secondary particles are generated at the decay point. The decay also happens via a NC weak interaction.
The distance is chosen from an ad-hoc power-law distribution, with minimum and maximum values that maximize the selection efficiency in this analysis. The physically correct distribution will be achieved through the re-weighting process described in \cref{sec:weighting}.
...
The mass of the HNL is a free parameter in the model under investigation. As motivated in the $\nu$MSM~\cite{ASAKA200517, ASAKA2005151}, extending the SM by three right-handed neutrinos, where two of them are heavy and one of them is light, could explain both the origin of neutrinos masses, while also providing a dark matter candidate. This work will therefore focus on HNLs in the GeV regime.
...
The generator additionally supports an HNL mass down to \SI{0.1}{\gev}. The current analysis scheme, however, turned out to be less sensitive to this mass and was not pursued to the end.
...
\begin{figure}[h]
    ...
    \internallinenumbers
    \caption{Shown are decay widths of the HNL within the mass range considered in this work, assuming mixing only with the $\tau$ sector. Since the decay widths scale linearly with the mixing, it can be factored out and the plot shows the decay widths for a mixing of $\nomathut4=1$ to visualize their shape. Calculated based on the results from Ref.~\cite{Coloma:2020lgy}.}
...
\begin{equation}
    \tau_\mathrm{proper} = \DIFdelbegin \DIFdel{\frac{\hbar}{\Gamma_\mathrm{total}(m_4) \cdot |U_{\tau4}|^2}
    }\DIFdelend \DIFaddbegin \DIFadd{\frac{\hbar}{\Gamma_\mathrm{total}(m_4, \nomathut4)} = \frac{\hbar}{\Gamma^*_\mathrm{total}(m_4) \cdot \nomathut4}
    }\DIFaddend \;.
    \label{eq:proper_lifetime}
\end{equation}
Here $\hbar$ is the reduced Planck constant, \DIFdelbegin \DIFdel{$\Gamma_\mathrm{total}(m_4)$ }\DIFdelend \DIFaddbegin \DIFadd{$\Gamma_\mathrm{total}(m_4, \nomathut4)$ }\DIFaddend the total decay width for a specific HNL mass \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{and mixing strength, }\DIFaddend \ut4\DIFdelbegin \DIFdel{the mixing strength}\DIFdelend \DIFaddbegin \DIFadd{, while $\Gamma^*_\mathrm{total}(m_4)$ is the same quantity with the mixing factored out}\DIFaddend .
...
\begin{figure}[h]
    ...
    \internallinenumbers
    \caption{Selection efficiency of the filter levels aiming at reducing atmospheric $\mu$'s and noise. Shown are the rates of all standard model backgrounds as well as the HNL rates for the three mass samples at a mixing of $\nomathut4 = 0.1$.}
...
\begin{align*}
    \DIFdelbegin \DIFdel{|U_{\tau4}|^2}\DIFdelend \DIFaddbegin \nomathut4\DIFaddend (\SI{0.3}{\gev}) &= 0.003 \;, \\
    \DIFdelbegin \DIFdel{|U_{\tau4}|^2}\DIFdelend \DIFaddbegin \nomathut4\DIFaddend (\SI{0.6}{\gev}) &= 0.080 \;, \rm{and} \\
    \DIFdelbegin \DIFdel{|U_{\tau4}|^2}\DIFdelend \DIFaddbegin \nomathut4\DIFaddend (\SI{1.0}{\gev}) &= 0.106 \;.
\end{align*}
...
\begin{table}[h]
    ...
    \hline\hline
    \textbf{Parameter} & \textbf{Nominal} & \DIFdelbeginFL %DIFDELCMD < \multicolumn{3}{c}{\textbf{Best-Fit / Pull}} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{3}{c}{\textbf{Best-Fit / Pull [\si{\sigma}]}} \DIFaddendFL \\ 
    ...
    \hline
    \end{tabular}
\caption{Best fit nuisance parameters for the three mass samples. Shown are the nominal values, the best-fit values for parameters without a prior, \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{or }\DIFaddendFL the pull \DIFaddbeginFL \DIFaddFL{in units of $\sigma$ }\DIFaddendFL for parameters that have a (Gaussian) prior.}
...
Unfortunately, given the low light yield and the detector performance, with the current tools we are not able to successfully identify low-energy double-cascade events against the background from SM atmospheric neutrino interactions.
The fact that we presently cannot single out double-cascade events from the background implies that the sensitive region obtained by assuming an order one detected even presented in~\cite{Coloma:2017ppo} is significantly different from the sensitive region.
...
$\nomathut4 < 0.19\;(m_4 = \SI{0.3}{\gev})$, $\nomathut4 < 0.36\;(m_4 = \SI{0.6}{\gev})$, and $\nomathut4 < 0.40\;(m_4 = \SI{1.0}{\gev})$ at \SI{90}{\percent} confidence level.
